import torch
import torch.nn as nn

class ComplexConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=(5, 2), stride=(2, 1), padding=(2, 1)):
        super().__init__()
        self.real_conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding)
        self.imag_conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding)
        
    def forward(self, x_r, x_i):
        out_r = self.real_conv(x_r) - self.imag_conv(x_i)
        out_i = self.imag_conv(x_r) + self.real_conv(x_i)
        return out_r, out_i

class ComplexConvTranspose2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=(5, 2), stride=(2, 1), padding=(2, 1), output_padding=(1, 0)):
        super().__init__()
        self.real_convt = nn.ConvTranspose2d(in_channels, out_channels, kernel_size, stride, padding, output_padding)
        self.imag_convt = nn.ConvTranspose2d(in_channels, out_channels, kernel_size, stride, padding, output_padding)
        
    def forward(self, x_r, x_i):
        out_r = self.real_convt(x_r) - self.imag_convt(x_i)
        out_i = self.imag_convt(x_r) + self.real_convt(x_i)
        return out_r, out_i

class DCCRN_SOTA(nn.Module):
    """
    State-of-the-Art 6-Layer Deep Complex Convolutional Recurrent Network.
    Designed for production deployment on AMD FPGAs.
    """
    def __init__(self):
        super().__init__()
        self.quant = torch.ao.quantization.QuantStub()
        self.dequant = torch.ao.quantization.DeQuantStub()

        # 6-Layer Deep Encoder
        self.enc1 = ComplexConv2d(1, 16)
        self.enc2 = ComplexConv2d(16, 32)
        self.enc3 = ComplexConv2d(32, 64)
        self.enc4 = ComplexConv2d(64, 128)
        self.enc5 = ComplexConv2d(128, 256)
        self.enc6 = ComplexConv2d(256, 256)
        
        # 6-Layer Deep Decoder
        self.dec6 = ComplexConvTranspose2d(256, 256, output_padding=(0, 0))
        self.dec5 = ComplexConvTranspose2d(256, 128, output_padding=(0, 0))
        self.dec4 = ComplexConvTranspose2d(128, 64, output_padding=(0, 0))
        self.dec3 = ComplexConvTranspose2d(64, 32, output_padding=(0, 0))
        self.dec2 = ComplexConvTranspose2d(32, 16, output_padding=(0, 0))
        self.dec1 = ComplexConvTranspose2d(16, 1, output_padding=(0, 0))

    def forward(self, x):
        x_r, x_i = x[:, 0:1, :, :], x[:, 1:2, :, :]
        x_r, x_i = self.quant(x_r), self.quant(x_i)
        
        e1_r, e1_i = self.enc1(x_r, x_i)
        e2_r, e2_i = self.enc2(e1_r, e1_i)
        e3_r, e3_i = self.enc3(e2_r, e2_i)
        e4_r, e4_i = self.enc4(e3_r, e3_i)
        e5_r, e5_i = self.enc5(e4_r, e4_i)
        e6_r, e6_i = self.enc6(e5_r, e5_i)
        
        d6_r, d6_i = self.dec6(e6_r, e6_i)
        d5_r, d5_i = self.dec5(d6_r, d6_i)
        d4_r, d4_i = self.dec4(d5_r, d5_i)
        d3_r, d3_i = self.dec3(d4_r, d4_i)
        d2_r, d2_i = self.dec2(d3_r, d3_i)
        d1_r, d1_i = self.dec1(d2_r, d2_i)
        
        out_r, out_i = self.dequant(d1_r), self.dequant(d1_i)
        return torch.cat([out_r, out_i], dim=1)
